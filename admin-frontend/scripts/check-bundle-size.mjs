...
if (!fs.existsSync(distDir)) {
    console.error('[admin-bundle-budget] dist/ does not exist. Run `npm run build` first.');
    process.exit(1);
}
...
const budgets = [
    {
        key: 'totalJs',
        label: 'Admin total JS',
        bytes: totalJsBytes,
        budget: 560 * 1024,
    },
    {
        key: 'largestJs',
        label: 'Admin largest JS chunk',
...
console.log('[admin-bundle-budget] Generated bundle size report:', path.relative(process.cwd(), reportPath));
...
if (failed.length > 0) {
    console.error('[admin-bundle-budget] Build exceeds configured budget(s):');
...
console.log('[admin-bundle-budget] All bundle budgets are within thresholds.');
