...
export function SeoToolsModule() {
    ...
    const noindexCount = useMemo(
        () => rows.filter((item) => item.seo?.indexPolicy === 'noindex').length,
        [rows]
    );
    ...
    return (
        <ModuleScaffold
            ...
            title="SEO Tools"
            description="Manage meta, canonical, index policy, and structured data on each post."
            meta={<span>{selected ? `Editing ${selected.title}` : 'Select a post to edit SEO metadata.'}</span>}
            metrics={[
                { key: 'seo-total', label: 'Matching Posts', value: rows.length },
                { key: 'seo-selected', label: 'Selected', value: selected ? 1 : 0, tone: selected ? 'info' : 'neutral' },
                { key: 'seo-noindex', label: 'Noindex', value: noindexCount, tone: noindexCount > 0 ? 'warning' : 'neutral' },
            ]}
            filters={{
                controls: (
                    <input
        ...
        >
            <div className="ops-stack">
                ...
                {selected ? (
                    <OpsCard title="SEO Editor" description="Update meta, schema, canonical, and index posture for the selected announcement." tone="muted">
