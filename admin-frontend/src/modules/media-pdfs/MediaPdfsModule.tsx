...
export function MediaPdfsModule() {
    ...
    const archivedCount = useMemo(() => rows.filter((item) => item.status === 'archived').length, [rows]);
    ...
    return (
        <ModuleScaffold
            eyebrow="Publishing"
            title="Media / PDFs"
            description="Track notification PDFs and media references with stable URL control."
            meta={<span>{rows.length} assets in the current library view.</span>}
            metrics={[
                { key: 'media-total', label: 'Total Assets', value: rows.length },
                { key: 'media-active', label: 'Active', value: activeCount, tone: activeCount > 0 ? 'success' : 'neutral' },
                { key: 'media-archived', label: 'Archived', value: archivedCount, tone: archivedCount > 0 ? 'warning' : 'neutral' },
            ]}
            filters={{
                controls: (
                    <>
        ...
        >
            <div className="ops-stack">
                <OpsCard title="Register Asset" description="Store PDFs and media references with stable URL behavior.">
